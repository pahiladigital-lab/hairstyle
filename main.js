const CONFIG = {
  ...
  BUSINESS_NAME: 'Hair Style Barber Shop',
  BUSINESS_PHONE: '9806690549',
...
function renderSlots() { const times = ['10:00 AM', '11:00 AM', '12:00 PM', '1:00 PM', '2:00 PM', '3:00 PM', '4:00 PM', '5:00 PM', '6:00 PM', '7:00 PM']; $('#slot-grid').innerHTML = times.map((time) => { const isFull = state.booking.unavailable.includes(time); return `<button class="slot ${isFull ? 'unavailable' : ''}" ${isFull ? 'disabled' : ''} data-time="${time}">${isFull ? `<span class="slot-time-struck">${time}</span><span class="slot-full-badge">Fully Booked</span>` : time}</button>`; }).join(''); $$('#slot-grid .slot:not(.unavailable)').forEach((button) => button.addEventListener('click', () => chooseBookingTime(button.dataset.time))); }
...
async function submitBooking(event) { event.preventDefault(); const form = $('#booking-form'); const button = form.querySelector('button'); const message = $('#booking-message'); const data = { businessId: CONFIG.BUSINESS_ID, businessName: CONFIG.BUSINESS_NAME, businessPhone: CONFIG.BUSINESS_PHONE, date: state.booking.date, time: state.booking.time, name: $('#booking-name').value.trim(), phone: $('#booking-phone').value.trim(), email: $('#booking-email').value.trim(), service: $('#booking-service').value }; button.disabled = true; button.textContent = 'Confirming…'; message.textContent = ''; try { const result = await requestJson(CONFIG.BOOKING_SCRIPT_URL, data); if (result.booked === false || result.success === false || result.error === 'SLOT_TAKEN') { message.textContent = 'That slot was just booked — please pick another.'; state.booking.unavailable.push(state.booking.time); $('#booking-details-view').classList.add('hidden'); $('#booking-slots-view').classList.remove('hidden'); updateBookingStep(2); renderSlots(); return; } $('#booking-details-view').classList.add('hidden'); $('#booking-success-view').classList.remove('hidden'); $('#booking-success-copy').textContent = `${data.date} at ${data.time} is reserved for ${data.name}. We will see you soon.` + (data.email ? ' A confirmation email is on its way.' : ''); } catch (error) { message.textContent = 'We could not confirm that just yet. Please try again.'; } finally { button.disabled = false; button.innerHTML = 'Confirm Booking <i data-lucide="arrow-up-right"></i>'; lucide.createIcons(); } }
